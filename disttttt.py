...
mongo_client2=mc(url2,appname="wt_mongo_client2",connect=False,maxPoolSize=500)
...
for i in range(len(docs)):
    ans =0
    rss = ((docs[i]['rss']))
    callingstationid = str(((docs[i]['callingstationid'])))
    ans = str((10 ** ((-69 - (rss)) / (10 * 2))))
    mydictionary['callingstationid']=callingstationid
    mydictionary['ans']=ans
    collection.insert_one(mydictionary)
    callingstationid.pop(0)
    i=i+1
